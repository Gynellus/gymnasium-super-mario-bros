...
_PARENT_DIR = os.path.dirname(_MODULE_PATH)
# the pattern to find the C++ shared object library
_SO_PATH = os.path.join('nes_py', 'lib_nes_env*')
# the absolute path to the C++ shared object library
_LIB_PATH = os.path.join(_PARENT_DIR, _SO_PATH)
...
class NESEnv(gymnasium.Env):
    ...
    def __init__(self, rom_path, render_mode='rgb_array'):
        ...
        # store the render mode for the environment
    ...
    def reset(self, seed=None, options=None, return_info=None, set_render_mode=None):
        """
        ...
        """
        # Set the seed. (https://gymnasium.farama.org/content/migration-guide/#environment-reset)
        ...
        self.done = False
        # if set_render_mode is not None, set the render mode
        if set_render_mode is not None:
            self.render_mode = set_render_mode
        # render if the environment is in human mode
